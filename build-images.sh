...
# Reuse existing nodebuilder-arkcase container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-arkcase; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-arkcase -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/mariadb:10.11.5 docker.io/geniusdynamics/arkcase:latest" \
    "${container}"
# Commit the image
